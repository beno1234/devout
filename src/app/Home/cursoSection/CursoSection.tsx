  ...
  return (
    <section className="p-64">
      ...
      <div className="w-full">
        ...
        >
          {cards.map((card) => (
            <SwiperSlide key={card.id} className="flex items-stretch">
              <div className="relative bg-white text-gray-700 shadow-md hover:shadow-lg transition-shadow rounded-xl p-6 border border-gray-200 flex flex-col justify-between min-h-[220px]">
                <div className="absolute top-3 right-3 bg-blue-500 text-white text-sm px-3 py-1 rounded-md shadow-sm">
                  {card.pontos}
                </div>
                <h2 className="text-lg md:text-xl font-semibold">
                  {card.title}
                </h2>
                <p className="text-gray-600 text-sm md:text-base line-clamp-3 overflow-hidden">
                  {card.description}
                </p>
              </div>
            </SwiperSlide>
